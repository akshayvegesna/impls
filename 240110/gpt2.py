...
class GPTBigCodeAttention(nn.Module):
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        print('>>attn: query, key, value: ', query.shape, key.shape, value.shape)
        dtype = query.dtype
        ...
        print('>>batch_size, key_length: ', batch_size, key_length)
        if self.multi_query:
            ...
            print('>>attn_shape, attn_view: ', attn_shape, attn_view)
...
class GPTBigCodeBlock(nn.Module):
    ...
    def forward(
        ...
        print('>>hidden_states: ', hidden_states.detach().cpu())
        print('>>hidden_states.norm: ', hidden_states.norm().detach().cpu())
        exit() 
...
class GPTBigCodeModel(GPTBigCodePreTrainedModel):
    ...
    def forward(
        ...
        print('self.wte', self.wte.weight.shape)
        print('self.wpe', self.wpe.weight.shape)
...
class GPTBigCodeForCausalLM(GPTBigCodePreTrainedModel):
    ...
    def forward(
        ...
        print('>>lm_logits: ', lm_logits.detach().cpu())
        print('>>lm_logits.norm: ', lm_logits.norm().detach().cpu())
        exit() 
...
if __name__ == '__main__': 
    ...
    # greedy generation 
    for i in range(1):
        outputs = model(input_ids=seq).logits
        next_token_logits = outputs[:, -1, :]
        next_token_id = torch.argmax(next_token_logits, dim=-1)
        seq = torch.cat([seq, next_token_id.unsqueeze(-1)], dim=-1)
